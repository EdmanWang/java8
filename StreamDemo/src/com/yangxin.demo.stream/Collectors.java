...
public final class Collectors {
    ...
    /**
     * 返回一个合并函数，适用于{@link Map#merge(Object, Object, BiFunction) Map.merge()}
     * 或者{@link #toMap(Function, Function, BinaryOperator) toMap()}
    ...
    /**
     * 使用ArrayList作为收集容器，来存储数据，并且拥有list的特性
     */
    public static <T>
    Collector<T, ?, List<T>> toList() {
        return new CollectorImpl<>((Supplier<List<T>>) ArrayList::new,  // supplier 部分
                List::add,// accumulator 累加器部分
                (left, right) -> { left.addAll(right); return left; },// combiner部分
                CH_ID);
    }

    /**
     * 是HashSet集合作为结果容器来存储数据，并且拥有set的特性
     */
    public static <T>
    ...
    /**
     * 把流中的数据拼接，并且没有分割的类型，底层实现是使用StringBuilder结果来操作的。
     *
     * @param CharSequence 是字符序列接口，String、Stringbuilder等都是实现该接口。
     */
    ...
    /**
     * 这个字符拼接，使用的hiStringJoiner 来进行操作的
     */
    public static Collector<CharSequence, ?, String> joining(CharSequence delimiter,
                                                             CharSequence prefix,
                                                             CharSequence suffix) {
    ...
    /**
     * {@code BinaryOperator<Map>} 将右边的内容合并到左边，使用提供的合并函数去处理重复的键
     *
     * @param <K> type of the map keys
     * @param <V> type of the map values
     * @param <M> type of the map
     * @param mergeFunction 合并函数，使用于{@link Map#merge(Object, Object, BiFunction) Map.merge()}
